...
def get_common_gov(pageBegin: int, pageEnd: int) -> list:
    ...
    for i in tqdm(range(pageBegin, pageEnd + 1)):
        ...
        url = f"http://www.jimei.gov.cn/ywkd/jmxw/index_{i}.htm"
        # 发起请求
        ...
        element_a_href = html.xpath('//div[@class="gl_list"]//a/@href')
        # TODO：替换成实际日期的xpath路径
        # element_span = html.xpath('//*[@id="resources"]/li/span/text()')
        element_span = html.xpath('//div[@class="gl_list"]//span/text()')
...
def get_detail_content(data: list) -> list:
    ...
    baseUrl = 'http://www.jimei.gov.cn/ywkd/jmxw/'
    # 遍历原列表中的数据
    for index, value in tqdm(enumerate(data)):
        ...
        try:
            ...
            element_div_content = html.xpath('//div[@class="TRS_Editor"]//text()')
            # 清洗数据，把空白符剔除掉
