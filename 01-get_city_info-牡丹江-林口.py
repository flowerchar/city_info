...
def get_common_gov(pageBegin: int, pageEnd: int) -> list:
    ...
    for i in tqdm(range(pageBegin, pageEnd + 1)):
        ...
        url = f"https://www.linkou.gov.cn/list.php?id=106&page={i}"
        # 发起请求
...
def get_detail_content(data: list) -> list:
    ...
    baseUrl = 'https://www.linkou.gov.cn/'
    # 遍历原列表中的数据
    for index, value in tqdm(enumerate(data)):
        ...
        try:
            ...
            element_div_content = html.xpath('//*[@id="pages_tr_0"]//text()')
            # 清洗数据，把空白符剔除掉
...
data = get_common_gov(169, 213)#213
