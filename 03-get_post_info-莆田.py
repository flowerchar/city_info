...
# TODO：这里填上具体城市的防盗链
headers = {'Referer': 'https://www.putian.gov.cn/zwgk/ptdt/ptyw/'}
...
def post_implicit_gov(pageBegin: int, pageEnd: int) -> list:
    ...
    for i in tqdm(range(144, 182)):

        # TODO：这里需要替换成实际城市的url
        url = f"https://www.putian.gov.cn/fjdzapp/search?channelid=201988&sortfield=-docorderpri%2C-docreltime&classsql=(chnlid%3D194)&classcol=publishyear&classnum=100&classsort=0&cache=true&page={i}&prepage=75"
        # 发起请求
        ...
        for j in json_data['data']:
            ...
            title = j['doctitle']
            # TODO：从字典里取出日期
            date = j['opertime'].split()[0].replace('.', '-')
            # TODO：取出超链接
            href = j['docpuburl']
            # TODO：取出文本内容
...
def get_detail_info(data: list) -> list:
    ...
    for i, v in tqdm(enumerate(filter_data)):
        ...
        content = html.xpath('//*[@id="detailCont"]//div[@class="TRS_Editor"]//text()')
        # 清洗数据，把空白符剔除掉
