...
def get_common_gov(pageBegin: int, pageEnd: int) -> list:
    ...
    for i in tqdm(range(pageBegin, pageEnd + 1)):
        ...
        url = f"https://www.mdjxa.gov.cn/index.php?c=article&a=type&tid=126&page={i}"
        # 发起请求
        ...
        element_a_text = html.xpath('//ul[@class="jgzn_list"]//a/text()')
        # TODO：替换成实际词条的href属性
        # element_a_href = html.xpath('//*[@id="resources"]/li/a/@href')
        element_a_href = html.xpath('////ul[@class="jgzn_list"]//a/@href')
        # TODO：替换成实际日期的xpath路径
        # element_span = html.xpath('//*[@id="resources"]/li/span/text()')
        element_span = html.xpath('////ul[@class="jgzn_list"]//span/text()')
...
def get_detail_content(data: list) -> list:
    ...
    baseUrl = 'https://www.mdjxa.gov.cn/'
    # 遍历原列表中的数据
    for index, value in tqdm(enumerate(data)):
        ...
        try:
            ...
            element_div_content = html.xpath('//*[@id="dong"]//text()')
            # 清洗数据，把空白符剔除掉
