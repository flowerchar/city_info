...
def get_common_gov(pageBegin: int, pageEnd: int) -> list:
    ...
    for i in tqdm(range(pageBegin, pageEnd + 1)):
        ...
        element_a_text = html.xpath('//*[@id="11a6765c3ab0406e874bd3f994dc88ce"]/div[2]/ul/li/a/text()')
        # TODO：替换成实际词条的href属性__已完成
        # element_a_href = html.xpath('//*[@id="11a6765c3ab0406e874bd3f994dc88ce"]/div[2]/ul/li[2]/a/@href')
        element_a_href = html.xpath('//*[@id="11a6765c3ab0406e874bd3f994dc88ce"]/div[2]/ul/li/a/@href')
        # TODO：替换成实际日期的xpath路径__已完成
        # element_span = html.xpath('//*[@id="11a6765c3ab0406e874bd3f994dc88ce"]/div[2]/ul/li[2]/span/text()')
        element_span = html.xpath('//*[@id="11a6765c3ab0406e874bd3f994dc88ce"]/div[2]/ul/li/span/text()')
...
def get_detail_content(data: list) -> list:
    ...
    """
    # TODO：此处改写成你所爬取的城市名__已完成
    cityName = "锡林郭勒"
    # 存放进CSV的列表数据
    fullData: list = []
    # TODO：参见README中获得baseUrl的方法__已完成
    baseUrl = 'https://www.xlgl.gov.cn'
    # 遍历原列表中的数据
    for index, value in tqdm(enumerate(data)):
        ...
        # 转换成lxml解析树
        html = etree.HTML(response.text)
        # TODO：这里替换成实际城市中的xpath__已完成
        element_div_content = html.xpath('//*[@id="para"]/div/div//text()')
        # 清洗数据，把空白符剔除掉
...
if __name__ == '__main__':
    # 得到总览页的数据信息 TODO：这里改写成实际的城市起止页__已完成
    data = get_common_gov(267, 268)
    # 过滤掉非2018年的数据
    data1 = filter_by_time(data)
    # 获取详情页的文本数据
    data2 = get_detail_content(data1)
    # 按照日期时间升序
    data3 = filter_by_time(data2)
    # 保存文件到csv TODO：这里替换成实际的省份-城市名
    keep2csv("内蒙古_锡林郭勒",data3)
